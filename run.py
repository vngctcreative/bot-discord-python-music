...
def finalize_logging():
    if os.path.isfile("logs/dcs_music_bot.log"):
        log.info("Di chuyển nhật ký DCS Music BOT cũ")
        try:
            if os.path.isfile("logs/dcs_music_bot.log.last"):
                os.unlink("logs/dcs_music_bot.log.last")
            os.rename("logs/dcs_music_bot.log", "logs/dcs_music_bot.log.last")
        except:
            pass

    with open("logs/dcs_music_bot.log", "w", encoding="utf8") as f:
        tmpfile.seek(0)
    ...
    fh = logging.FileHandler("logs/dcs_music_bot.log", mode="a")
    fh.setFormatter(
        logging.Formatter(
...
def sanity_checks(optional=True):
    log.info("Bắt đầu kiểm tra mã nguồn.")
    ## Required
    ...
    log.info("Các kiểm tra cần thiết đã thành công !!!")
    ...
    log.info("Các kiểm tra tùy chọn đã thành công !!!")

def req_ensure_py3():
    log.info("Kiểm tra phiên bản Python")
...
def req_ensure_encoding():
    log.info("Kiểm tra mã hóa bảng điều khiển")

    if (
        sys.platform.startswith("win")
        or sys.stdout.encoding.replace("-", "").lower() != "utf8"
    ):
        log.info("Đặt mã hóa bảng điều khiển thành UTF-8")
...
def req_ensure_env():
    log.info("Đảm bảo rằng chúng ta đang ở trong môi trường đúng")
    ...
    if sys.platform.startswith("win"):
        log.info("Thêm thư mục cục bộ bins/folder vào đường dẫn")
