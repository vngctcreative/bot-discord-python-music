...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None, aliases_file=None):
        try:
            sys.stdout.write("\x1b]2;DCS Music BOT\x07")
        except:
            pass
        ...
        log.info("Khởi động DCS Music BOT")

        if not self.autoplaylist:
            log.warning("Danh sách tự động phát trống, đang tắt.")
            self.config.auto_playlist = False
        else:
            log.info(
                "Đã tải danh sách tự động phát có {} mục nhập".format(len(self.autoplaylist))
            )

        if self.blacklist:
            log.debug("Đã tải danh sách đen có {} mục nhập".format(len(self.blacklist)))
        ...
        if self.config._spotify:
            try:
                ...
                if not self.spotify.token:
                    log.warning("Spotify không cung cấp cho chúng tôi Token. Vô hiệu hóa.")
                    self.config._spotify = False
                else:
                    log.info(
                        "Đã xác thực thành công với Spotify bằng ID Client và Secret."
                    )
            except exceptions.SpotifyError as e:
                log.warning(
                    "Đã xảy ra sự cố khi khởi tạo kết nối với Spotify. ID Client và Secret của bạn có đúng không? Chi tiết: {0}. Vẫn tiếp tục trong 5 giây...".format(
                        e
                    )
                )
                self.config._spotify = False
            ...
            try:
                log.warning(
                    "Cấu hình không có thông tin xác thực ứng dụng Spotify, đang cố gắng sử dụng chế độ khách."
                )
                self.spotify = Spotify(
                ...
                if not self.spotify.token:
                    ...
                    log.info(
                        "Đã xác thực thành công với Spotify bằng chế độ khách."
                    )
                    self.config._spotify = True
            except exceptions.SpotifyError as e:
                log.warning(
                    "Đã xảy ra sự cố khi khởi tạo kết nối với Spotify bằng chế độ khách. Chi tiết: {0}.".format(
                        e
                    )
                )
                self.config._spotify = False
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not orig_msg or orig_msg.author.id == self.config.owner_id:
                ...
                raise exceptions.PermissionsError(
                    "Chỉ chủ sở hữu mới có thể sử dụng lệnh này.", expire_in=30
                )
    ...
    def dev_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if str(orig_msg.author.id) in self.config.dev_ids:
                ...
                raise exceptions.PermissionsError(
                    "Chỉ người điều hành mới có thể sử dụng lệnh này.", expire_in=30
                )
    ...
    def _setup_logging(self):
        ...
        log.debug("Đặt mức ghi nhật ký thành {}".format(self.config.debug_level_str))
    ...
    async def _join_startup_channels(self, channels, *, autosummon=True):
        ...
        for guild in self.guilds:
            ...
            if guild.me.voice:
                log.info(
                    "Đã tìm thấy kênh thoại có thể tiếp tục {0.guild.name} | {0.name}".format(
            ...
            if autosummon:
                ...
                if owner:
                    log.info('Đã tìm thấy chủ sở hữu BOT ở kênh #{}'.format(owner.voice.channel.name))
                    channel_map[guild] = owner.voice.channel

        for guild, channel in channel_map.items():
            if guild in joined_servers:
                log.info(
                    'Đã tham gia một kênh trong "{}", bỏ qua'.format(guild.name)
                )
                continue

            if channel and isinstance(channel, discord.VoiceChannel):
                log.info("Đang cố gắng tham gia {0.guild.name} | {0.name}".format(channel))

                chperms = channel.permissions_for(guild.me)

                if not chperms.connect:
                    log.info(
                        'Không thể tham gia kênh "{}", không có quyền.'.format(channel.name)
                    )
                    continue

                elif not chperms.speak:
                    log.info(
                        'Sẽ không tham gia kênh "{}", không có quyền nói.'.format(
                            channel.name
                        )
                    )
                    continue
                ...
                try:
                    ...
                    log.info("Đã tham gia {0.guild.name} | {0.name}".format(channel))
                    ...
                    log.debug(
                        "Lỗi khi tham gia {0.guild.name} | {0.name}".format(channel),
                        exc_info=True,
                    )
                    log.error("Lỗi khi tham gia {0.guild.name} | {0.name}".format(channel))

            elif channel:
                log.warning(
                    "Không tham gia {0.guild.name} | {0.name}, đây là kênh văn bản.".format(
                        channel
                    )
                )
    ...
    async def on_player_finished_playing(self, player, **_):
        ...
        if not player.playlist.entries and not player.current_entry and self.config.auto_playlist:
            ...
            while player.autoplaylist:
                song_url = random.choice(player.autoplaylist) if self.config.auto_playlist_random else player.autoplaylist[0]
    ...
    async def safe_send_message(self, dest, content, **kwargs):
        ...
        try:
            ...
            lfunc('Cannot send message to "%s", không có quyền', dest.name)
    ...
    async def safe_delete_message(self, message, *, quiet=False):
        ...
        try:
            ...
            lfunc('Cannot delete message "{}", không có quyền'.format(message.clean_content))
    ...
    async def on_ready(self):
        ...
        if self.config.bound_channels:
            ...
            if chlist:
                ...
                print("Không liên kết với bất kỳ kênh văn bản nào")
            ...
            log.info("Không liên kết với bất kỳ kênh văn bản nào")
        ...
        if self.config.autojoin_channels:
            ...
            if chlist:
                ...
                log.info("Không tự động tham gia bất kỳ kênh thoại nào.")
    ...
    def _gen_embed(self):
        ...
        e.set_author(
            name=self.user.name,
            url="https://github.com/VngGodCreative",
    ...
    async def cmd_summon(self, channel, guild, author, voice_channel):
        ...
        if voice_client and guild == author.voice.channel.guild:
            ...
            if not chperms.connect:
                log.warning(
                    "Không thể tham gia kênh '{0}', không có quyền.".format(
                ...
                raise exceptions.CommandError(
                    self.str.get(
                        ...
                        "Không thể tham gia kênh `{0}`, không có quyền kết nối.",
                ...
                log.warning(
                    "Không thể tham gia kênh '{0}', không có quyền nói.".format(
                        author.voice.channel.name
                    )
                )
                raise exceptions.CommandError(
                    self.str.get(
                        ...
                        "Không thể tham gia kênh `{0}`, không có quyền nói.",
                    ).format(author.voice.channel.name),
                    expire_in=25,
                )
    ...
    async def cmd_setname(self, leftover_args, name):
        ...
        return Response(
            "Đặt tên người dùng của bot thành **{0}**".format(name), delete_after=20
        )
    ...
    async def cmd_setnick(self, guild, channel, leftover_args, nick):
        ...
        if not channel.permissions_for(guild.me).change_nickname:
            raise exceptions.CommandError("Không thể thay đổi biệt hiệu: không có quyền.")
        ...
        return Response("Đặt biệt danh của bot thành `{0}`".format(nick), delete_after=20)
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        return Response("Đã thay đổi ảnh đại diện của bot", delete_after=20)
    ...
    async def cmd_disconnect(self, guild):
        ...
        return Response("Đã ngắt kết nối khỏi `{0.name}`".format(guild), delete_after=20)
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        autopause_msg = "{state} ở nhóm {channel.guild.name} | {channel.name} {reason}"
                ...
                if not auto_paused and player.is_playing:
                    log.info(
                        autopause_msg.format(
                            state="Tạm dừng nhạc",
                            channel=player.voice_client.channel,
                            reason="(kênh trống hoặc thành viên tắt nghe)",
                        ).strip()
                    )
    ...
    async def on_guild_join(self, guild: discord.Guild):
        log.info("Bot đã được thêm vào nhóm: {}".format(guild.name))
        owner = self._get_owner(voice=True) or self._get_owner()
        if self.config.leavenonowners:
            check = guild.get_member(owner.id)
            if check is None:
                await guild.leave()
                log.info("Thoát {} do không tìm thấy chủ sở hữu bot.".format(guild.name))
                await owner.send(
                    self.str.get(
    ...
    async def on_guild_remove(self, guild: discord.Guild):
        ...
        log.debug("Đã cập nhật danh sách nhóm:")
    ...
    async def on_guild_available(self, guild: discord.Guild):
        ...
        log.debug('Nhóm "{}" đã có sẵn.'.format(guild.name))
